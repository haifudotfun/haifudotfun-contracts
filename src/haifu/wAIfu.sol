...
contract wAIfu is ERC20, AccessControl, Initializable {
    // Define roles
    ...
    function openInfo() external view returns (IHaifu.wAIfuOpenInfo memory) {
        return IHaifu.wAIfuOpenInfo({
            creator: creator,
    ...
    {
        require(info.fundAcceptingExpiaryDate > block.timestamp, "Fund raising is expired");
        // deposit is obviously wAIfu token
        // get converted wAIfu token to deposit amount
        ...
        returns (IHaifu.OrderInfo memory depositOrderInfo, IHaifu.OrderInfo memory haifuOrderInfo, uint256 leftHaifu)
    {
        ...
        // 10% of the deposit token will be placed as bid order in {wAIfu token}/{deposit token} pair, and rest will be sent to fund manager address.
        ...
        // Raised $HAIFU by $wAIfu token holders will be placed as bid order for {wAIfu token}/$HAIFU pair.
        ...
        // Rest of wAIfu token will be sent to creator address.
        ...
        returns (IHaifu.OrderInfo memory expireOrderInfo, bool expiredEarly)
    {
        ...
        if (expiredEarly) {
            // burn wAIfu token and only leave raised amount
        ...
        returns (address claim, uint256 claimed, bool expiredEarly)
    {
        ...
        // send wAIfu token to wAIfu token contract
